...
### Part 1: Setting up the connectivity.
#### 1. Create a new VPC.
...
<img src="../Kubernetes/images-eks/createVPC.png" width="700" />
...
<img src="../Kubernetes/images-eks/public-subnet1.png" width="700" />
...
<img src="../Kubernetes/images-eks/public-subnet2.png" width="700" />
...
<img src="../Kubernetes/images-eks/private-subnet1.png" width="700" />
...
<img src="../Kubernetes/images-eks/private-subnet2.png" width="700" />

#### 3. Create Internet Gateway

Create an internet gateway instance igw-xxx.

<img src="../Kubernetes/images-eks/igw.png" width="700" />

Attach it to the above VPC `anjana-vpc`.

<img src="../Kubernetes/images-eks/igw-attach-vpc.png" width="700" />

<img src="../Kubernetes/images-eks/igw-attach-vpc2.png" width="700" />
...
<img src="../Kubernetes/images-eks/nat-gw.png" width="700" />
...
<img src="../Kubernetes/images-eks/create-rt-public.png" width="700" />

Associate the public subnets created in Step 2 to this route table

<img src="../Kubernetes/images-eks/rt-public-subnet-association.png" width="700" />

Add a new default route (0.0.0.0/0) to point to the igw created in Step 3.

<img src="../Kubernetes/images-eks/rt-public-add-route-igw.png" width="700" />
...
<img src="../Kubernetes/images-eks/create-rt-private.png" width="700" />

Associate the private subnets created in Step 2 to this route table

<img src="../Kubernetes/images-eks/rt-private-subnet-association.png" width="700" />

Add a new default route (0.0.0.0/0) to point to the nat gateway created in
Step 4.

<img src="../Kubernetes/images-eks/rt-private-edit-route-natgw.png" width="700" />

### Part 2. Creating the EKS cluster.

#### 1. Create a new security group.

<img src="../Kubernetes/images-eks/create-sg.png" width="700" />
...
<img src="../Kubernetes/images-eks/eks-create-cluster-configure.png" width="700" />

Use the VPC and SG created in the connectivity steps.

<img src="../Kubernetes/images-eks/eks-create-networking.png" width="700" />

Keep the cluster endpoint access to be “Public”.

<img src="../Kubernetes/images-eks/eks-create-cluster-endpoint-access.png" width="700" />
...
<img src="../Kubernetes/images-eks/create-ng.png" width="700" />

<img src="../Kubernetes/images-eks/ng-compute-config.png" width="700" />

<img src="../Kubernetes/images-eks/ng-node-config.png" width="700" />

<img src="../Kubernetes/images-eks/ng-network-config.png" width="700" />

Check that the node group status is active.

### Part 3. Creating a transit gateway (if needed)

#### 1. Create a new transit gateway from the console.

<img src="./images-eks/TGcreate.png" width="700" />

#### 2. Transit Gateway Attachment

Create a new Transit Gateway Attachment and select the newly created TGW and attachment type VPC. Select the VPC to be attached. Either control plane or data-plane. Select the appropriate subnets as well.

<img src="./images-eks/TGAttachmentCreate.png" width="700" />

Repeat the above step for the remaining VPCs to be attached to the TGW.

#### 3. Transit Gateway Route Table.

Create a new Transit Gateway Route table and Add associations to the above attachments.

<img src="./images-eks/TGRTcreate.png" width="700" />

#### 4. Edit Route Tables created in Part 1 step 5

Edit the Public and Private Route tables created in initial steps of setting up VPC to add route for the TGW. This is to be done in all the VPCs connected to the TGW.


### Part 4. Setting up Kubeconfig on your machine to reach the EKS cluster created
...
## Reference:
[AWS Transit Gateway](https://theithollow.com/2018/12/12/setup-aws-transit-gateway/)